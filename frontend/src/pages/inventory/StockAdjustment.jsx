...
import { fetchInventorySnapshots, exportInventorySnapshots } from '../../api';
    ...
    useEffect(() => {
        const loadSnapshots = async () => {
            try {
                ...
                // Map the last 20 snapshots as adjustments to show some data
                const mappedAdjustments = snapshots.slice(-20).reverse().map(s => ({
                    id: s.id.substring(s.id.length - 8),
                    sku: `Product ${s.productId.substring(s.productId.length - 4)}`,
                    reason: 'Inventory Snapshot Capture',
                    amount: s.runningInventory, // just showing the amount for demo
                    time: new Date(s.snapshotDate).toLocaleString()
                }));
        ...
        <div className="page-wrapper" style={{ maxWidth: '800px', margin: '0 auto' }}>
            <div className="page-header" style={{ marginBottom: 32 }}>
                ...
                <div className="header-actions">
                    <button className="btn-secondary" onClick={() => exportInventorySnapshots()}>
                        <History size={16} /> View History
                    </button>
                    <button className="btn-secondary" onClick={() => window.alert('Bulk CSV upload is not enabled on this demo environment. Please use the manual adjustment form below.')}>
